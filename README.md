...
**On stage, the solver is Eldarica, not z3.** The technique — Constrained Horn
...
pulled, which is the point of the demo-day checklist.
...
If someone objects that Slither "isn't formal methods", agree enthusiastically.
Slither is a static analyser, Mythril is a bounded symbolic executor, and
neither can certify the fix. That is what the table is for.
...
fixtures/           committed known-good results for demo day
...
`out/results.json` is missing or truncated, so a catastrophic pipeline failure
still produces a page:

```bash
python3 tools/render.py --input /nonexistent    # still writes out/index.html
```

Refresh the fixture after a good run with `make fixture`.
